...
float processResults::fusion_1110Classes(FusionInfo *fInfo,statsInfo *stats)
{
    int est_len,LEMsize;
    ...
    {	
        ...
        est_len = TopLEM[0].classID; //TOP 1 LEM             
        if(est_len >= 3)
        {
        ...
        else			
        {
            ...
            {   
                ...
                if (len == est_len+1)
                {
                    p1 = fInfo->main->components[j]->DSM[0][i];
        ...
        est_len = TopLEM[1].classID; //TOP 2 LEM             
        if(est_len >= 3)
        {
        ...
        else
        {
            for(int i=0; i<(int)fInfo->main->components[j]->DSM[0].size();i++)                
            {    
                //cout << est_len+1 << " " << i << "of" << (int)fInfo->main->components[j]->DSM[0].size() << " PRED " << fInfo->main->components[j]->DSM[0][i].className << endl;
                int len=fInfo->main->components[j]->DSM[0][i].className.size();
                if (len == est_len+1)
                {
                    p2 = fInfo->main->components[j]->DSM[0][i];
                    //cout << "BEST " << est_len+1 << " " << i << " PRED " << fInfo->main->components[j]->DSM[DSM_ID][i].className << endl;
                    break;
                }
            }                
        ...
        //cout <<  est_len+1 << "(" << TopLEM[1].score << ") " << p2.className << " " <<  p2.score << endl;   
        if(fType == CNN_1110_DIGIT_ONLY) //NO LENGHT
        {
            pBest = fInfo->main->components[j]->DSM[0][0];
            pBestLen = TopLEM[0];
        }
        else if (fType == CNN_1110_TOP1 || TopLEM[0].score >= fInfo->fusionThreshold)
        {
            
            //pBest = fInfo->main->components[j]->DSM[0][0]; //NOLEM
            pBest = p1;
            pBestLen = TopLEM[0]; 								
        }
        else if(fType == CNN_1110_MAX_DIGIT)    
        {   
            if(p1.score >= p2.score) //MAX DIGIT ONLY
            {
                pBest = p1;
                pBestLen = TopLEM[0]; 								
            }
        ...
        else if(fType == CNN_1110_PROD)   
        {
            if(p1.score*TopLEM[0].score >= p2.score*TopLEM[1].score) //PROD            
            {
                pBest = p1;
                pBestLen = TopLEM[0]; 								
            }
		...
		//NO-LEM: Keeps the best digit prediction
		//pBest = fInfo->main->components[j]->DSM[0][0];
...
float processResults::fusion(FusionInfo *fInfo,statsInfo *stats)
{	
	int DSM_ID,LEMsize;
	Prediction pBest,pBestLen,pDig,pLen,p1,p2;	
	vector <Prediction> TopLEM;	
    ...
    {	
        ...
        if((int)pBest.className.size() == 1)
            fInfo->predictSizeComp.append("1");
        else if((int)pBest.className.size() == 2)
            fInfo->predictSizeComp.append("2");
        else if((int)pBest.className.size() == 3)
            fInfo->predictSizeComp.append("3");
...
int processResults::process( ImageInfo *result, statsInfo *stats, SHOW_TYPE flagShow, SAVE_TYPE flagSave, float fThreshold)
{
    ...
    if(fType == CNN_1110_MAX_DIGIT || fType == CNN_1110_TOP1 || fType == CNN_1110_PROD || fType == CNN_1110_DIGIT_ONLY)
        fInfo.predictionScore = fusion_1110Classes(&fInfo,stats);	
    else
...
void processResults::saveResult( FusionInfo *fInfo)
{
	string filename;
	size_t found;	
	int wSpace=50;
...
int processResults::process( vector <ImageInfo *> &imgsProcessed, statsInfo *stats, 
